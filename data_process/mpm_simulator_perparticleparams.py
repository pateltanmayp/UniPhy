...
class MPMSimulator:
    ...
    def __init__(self, dtype, dt, frame_dt, particle_layout, dx, inv_dx, n_particles, gravity=[0, -9.8, 0], material=elasticity, cuda_chunk_size=400, **kargs):
        ...
        self.material = ti.field(dtype=self.dtype)
        ...
        self.C = ti.Matrix.field(dim, dim, dtype=self.dtype)  # affine velocity field
        self.F = ti.Matrix.field(dim, dim, dtype=self.dtype)  # deformation gradient
        self.F_tmp = ti.Matrix.field(dim, dim, dtype=self.dtype)  # deformation gradient
        self.stress = ti.Matrix.field(self.dim, self.dim, dtype=self.dtype)
        ...
        self.sig_out = ti.Vector.field(dim, dtype=self.dtype) # for isotropic matrials
        ...
        print (frame_dt, dt, self.n_substeps[None])
    ...
    def compute_F_tmp(self, s: ti.i32):
        for p in range(self.n_particles[None]):
            if self.material[p]==self.viscous_fluid:
                self.F_tmp[p, s+1][0,0] = (1.0 + self.dt[None] * self.C[p, s].trace()) * self.F[p, s][0,0]
            else:
                self.F_tmp[p, s+1] = (ti.Matrix.identity(self.dtype, self.dim) + self.dt[None] * self.C[p, s]) @ self.F[p, s]

    @ti.kernel
    def svd(self, s: ti.i32):
        for p in range(self.n_particles[None]):
            self.U[p], self.sig[p], self.V[p] = ti.svd(self.F_tmp[p, s+1].cast(ti.f64))

    @ti.kernel
    def svd_grad(self, s:ti.i32):
        for p in range(self.n_particles[None]):
            self.F_tmp.grad[p, s+1] += self.backward_svd(self.U.grad[p].cast(ti.f64), self.sig.grad[p].cast(ti.f64), self.V.grad[p].cast(ti.f64), self.U[p].cast(ti.f64), self.sig[p].cast(ti.f64), self.V[p].cast(ti.f64))
    ...
    def project_F(self, s: ti.i32):
        for p in range(self.n_particles[None]):
            ...
            if self.material[p] == self.viscous_fluid:
                self.F[p, s+1][0,0] = ti.max(self.F_tmp[p, s+1][0,0], 0.05)
                # self.F[p, s+1][0,0] = self.F_tmp[p][0,0]
            elif self.material[p] == self.drucker_prager:
                epsilon = ti.log(sig)
            ...
            elif self.material[p] == self.von_mises:
                b_trial = sig ** 2
                epsilon = ti.log(sig)
                trace_epsilon = epsilon.sum()
                epsilon_hat = epsilon - (epsilon.sum() / self.dim)
    ...
    def p2g(self, s: ti.i32):
        ...
        for p in range(self.n_particles[None]):
            ...
            if self.material[p] == self.elasticity:
                J = new_F.determinant()
            ...
            elif self.material[p] == self.viscous_fluid:
                J = new_F[0,0]
    ...
    @ti.kernel
    def get_v(self, f: ti.i32, v: ti.types.ndarray()):
        local_index = (f * self.n_substeps[None]) % self.cuda_chunk_size
        for i in range(self.n_particles[None]):
            for d in ti.static(range(3)):
                v[i, d] = ti.cast(self.v[i, local_index][d], ti.f32)

    @ti.kernel
    def get_x_gt(self, step:ti.i32, x_t: ti.types.ndarray()):
        for i in range(self.n_particles[None]):
            for d in ti.static(range(3)):
                x_t[i, step, d] = ti.cast(self.x[i, step][d], ti.f32)

    @ti.kernel
    def get_v_gt(self, step:ti.i32, v_t: ti.types.ndarray()):
        for i in range(self.n_particles[None]):
            for d in ti.static(range(3)):
                v_t[i, step, d] = ti.cast(self.v[i, step][d], ti.f32)
    ...
    def get_C(self, step:ti.i32, C_t: ti.types.ndarray()):
        for p in range(self.n_particles[None]):
            for i in ti.static(range(3)):
                for j in ti.static(range(3)):
                    C_t[p, step, i, j] = ti.cast(self.C[p, step][i, j], ti.f32)  # self.F = 3 x 3

    @ti.kernel
    def get_Ftmp(self, step:ti.i32, Ftmp_t: ti.types.ndarray()):
        for p in range(self.n_particles[None]):
            for i in ti.static(range(3)):
                for j in ti.static(range(3)):
                    Ftmp_t[p, step, i, j] = ti.cast(self.F_tmp[p, step][i, j], ti.f32)  # self.F = 3 x 3

    @ti.kernel
    def get_stress(self, step:ti.i32, stress_t: ti.types.ndarray()):
        for p in range(self.n_particles[None]):
            for i in ti.static(range(3)):
                for j in ti.static(range(3)):
